...
public class GPUImageFilterTools {  
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type) {
        switch (type) {
            ...
            case MONOCHROME:
            	return new GPUImageMonochromeFilter(1.0f, new float[]{1.6f, 0.45f, 0.3f, 1.0f});
            case OPACITY:
...
//                return amatorka;
    ...
    private static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            ...
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.icon));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
