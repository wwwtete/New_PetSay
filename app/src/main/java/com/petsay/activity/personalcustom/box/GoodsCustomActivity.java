package com.petsay.activity.personalcustom.box;
 ...
 *商品定制信息
 */
public class GoodsCustomActivity extends BaseActivity implements NetCallbackInterface,OnClickListener{
	...
	protected void initView(){
		...
		for (int i = 0; i < mSpecDTOs.size(); i++) {
			...
			if (null!=specDTO.getValues()&&!specDTO.getValues().isEmpty()) {
				mSelectedSpecList.get(i).setValue(specDTO.getValues().get(0).getId());
				...
				customGoodsSpecsView.setPadding(10, 10, 10, 10);
				customGoodsSpecsView.setBackgroundColor(Color.WHITE);
...
//		mSelectedSpecValueDTO=specValueDTOs.get(0);
		...
		mTvOnePrice.setText("优惠价：￥"+(onePrice));
