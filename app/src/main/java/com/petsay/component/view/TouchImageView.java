...
public class TouchImageView extends ImageView {
	...
	Bitmap gintama;
	Bitmap bitmap;
	public TouchImageView(Activity activity,int rulerTop,Bitmap img) {
		super(activity);
		setBackgroundColor(Color.BLACK);
		this.rulerTop=rulerTop;
		
//		gintama = BitmapFactory.decodeResource(getResources(), R.drawable.aaaa);
		gintama=img;
		...
		int scaleH=bh*widthScreen/bw;
        bitmap=Bitmap.createScaledBitmap(gintama, widthScreen, scaleH, true);
		matrix = new Matrix();
		float[] values ={1,0,0,0,1,this.rulerTop,0,0,1};
//		float[] values={0,155,720,875};
...
//				matrix1.postRotate(rotation, mid.x, mid.y);// 旋轉
	...
	private boolean matrixCheck() {
		...
		// 图片现宽度
		double width = gintama.getWidth()*f[5];
		// 缩放比率判断
...
//		}
	...
	public Bitmap getCaptureBitmap(){
		Matrix matrix=new Matrix();
		matrix.mapRect(new RectF(0, 0, widthScreen, heightScreen));
		Bitmap bitmap = Bitmap.createBitmap(widthScreen, heightScreen,Config.ARGB_8888); // 背景图片
		Canvas canvas = new Canvas(bitmap); // 新建画布
		canvas.drawBitmap(gintama, matrix, null); // 画图片
		canvas.save(Canvas.ALL_SAVE_FLAG); // 保存画布
		canvas.restore();
//		Bitmap.createBitmap(arg0, arg1, arg2)
		return bitmap;
