 ...
 * @Description 翻书效果View
 */
public class FlipBookView extends FrameLayout {
...
//            PetsayLog.d("[onTouch]-> action=%s",event.getAction());
			if (v == FlipBookView.this && mAdapter != null) {
...
//						Log.d("PageWidget->DragToRight", "" + (currentPosition-1));
						...
						nextView = mAdapter.getView(nextPosition, nextView, null);
						nextViewTranscript = mAdapter.getView(nextPosition, nextViewTranscript, null);
					} else {
						if(currentPosition >= itemCount) { mCornerX = 0; mCornerY = 0; return false; }
...
//						Log.d("PageWidget->", "" + (currentPosition+1));
                    ...
                    if(turnListener != null)
                        turnListener.onTurnStart(currentPosition,nextPosition );
					isAnimated = false;
					mTouch.x = event.getX();
					mTouch.y = event.getY();
...
//        PetsayLog.d("[dispatchDraw]->-start");
        calcPoints();
...
//        PetsayLog.d("[dispatchDraw]->-stop");
	...
	private void createDrawable() {
        ...
        int[] test = { 0xFFFF0000, 0xFF00FF00 };
    ...
    /**
     * 为了易于控制滑屏控制，Android框架提供了 computeScroll()方法去控制这个流程。在绘制View时，会在draw()过程调用该方法。
     * 因此， 再配合使用Scroller实例，我们就可以获得当前应该的偏移坐标，手动使View/ViewGroup偏移至该处
    ...
    public void setCurrentPosition(int position){
        if(itemCount != 0 && position > -1 && position <= itemCount) {
            ...
            int temp = currentPosition == 0 ? currentPosition : currentPosition -1;
            currentView = mAdapter.getView(currentPosition, currentView, null);
            nextView = mAdapter.getView(temp, nextView, null);
            nextViewTranscript = mAdapter.getView(temp, nextViewTranscript, null);
//            mTouch.x = 0.01f;
//            mTouch.y = 0.01f;
//            mCornerX = 0;
//            mCornerY = 0;
...
//            startAnimation(300);
    ...
    private void onTurn(){
        if(turnListener != null) {
            turnListener.onTurnFinish(currentPosition);
        }
    }
